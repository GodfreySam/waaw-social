...
const dpUpload = async (req, res, next) => {
   let userId = req.user._id;
   if (!req.file) return res.status(400).json({ msg: 'Please upload an image' });
   // if (req.file.mimetype != 'image/*') return res.status(400).json({ msg: 'Only image is allowed' });
   ...
   const uploadedImage = await cloudinary.uploader.upload(req.file.path, {
				eager: [
					...
					{ height: 250, width: 200, crop: "fill" },
				],
			});

   if (!uploadedImage) return res.status(500).json({ msg: 'An error occurred' });
